...
pub struct SecretsCompat {
    config: Arc<dyn ClientConfig>,
}

impl SecretsCompat {
    pub fn new(config: Arc<dyn ClientConfig>) -> SecretsCompat {
        SecretsCompat { config }
    }

    /// GET /secrets
    /// List secrets
    /// Returns a list of secrets
    pub async fn secret_list<'a>(
        &self,
        params: Option<super::super::params::SecretList<'a>>,
    ) -> Result<Vec<super::super::models::SecretInfoReportCompat>, Error> {
    ...
    /// DELETE /secrets/{name}
    /// Remove secret
    pub async fn secret_delete(&self, name: &str) -> Result<(), Error> {
        let mut request_url = url::Url::parse(self.config.get_base_path())?;
    ...
    pub async fn secret_inspect(
        &self,
        name: &str,
    ) -> Result<super::super::models::SecretInfoReportCompat, Error> {
        let mut request_url = url::Url::parse(self.config.get_base_path())?;
