...
pub struct VolumesCompat {
    config: Arc<dyn ClientConfig>,
}

impl VolumesCompat {
    pub fn new(config: Arc<dyn ClientConfig>) -> VolumesCompat {
        VolumesCompat { config }
    }

    /// GET /volumes
    /// List volumes
    /// Returns a list of volume
    pub async fn volume_list<'a>(
        &self,
    ...
    /// GET /volumes/{name}
    /// Inspect volume
    pub async fn volume_inspect(&self, name: &str) -> Result<super::super::models::Volume, Error> {
        let mut request_url = url::Url::parse(self.config.get_base_path())?;
