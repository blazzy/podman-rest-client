...
impl Networks {
    pub fn new(config: Arc<dyn ClientConfig>) -> Networks {
        Networks { config }
    }
    ...
    pub async fn network_exists_libpod(&self, name: &str) -> Result<(), Error> {
        let mut request_url = url::Url::parse(self.config.get_base_path())?;
    ...
    pub async fn network_list_libpod<'a>(
        &self,
    ...
    pub async fn network_prune_libpod<'a>(
        &self,
